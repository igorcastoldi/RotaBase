...
import { Bike, Clock, Users, Tag, MapPin, Sparkles, Compass } from 'lucide-react';
...
export default function HomeMarketplace() {
  ...
  const [filtroAtivo, setFiltroAtivo] = useState('Novos'); // Começa selecionado na aba Novos
  ...
  // Função inteligente para unificar variações de nomes de regiões (ex: "Lagoa do Bacupari" casa com "Lagoa do Bacupari · RS")
  ...
  // Extrai as regiões únicas cadastradas pelos guias/empresas para criar as abas automaticamente
  ...
  return (
    <div className="min-h-screen bg-gray-950 font-sans text-gray-100 selection:bg-orange-500 selection:text-gray-950">
      {/* Top Bar / Navigation */}
      <header className="border-b border-gray-900 bg-gray-950/80 backdrop-blur-md sticky top-0 z-50 px-4 sm:px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-orange-500 rounded-sm flex items-center justify-center font-black text-gray-950 text-lg">
            R
          </div>
          <span className="font-extrabold tracking-wider text-lg text-gray-50">ROTABASE</span>
        </div>
        <div className="flex items-center gap-4">
          <Link 
            href="/dashboard/empresa/passeios" 
            className="text-xs uppercase tracking-widest text-gray-400 hover:text-orange-400 transition font-semibold"
          >
            Área da Empresa
          </Link>
          <Link 
            href="/dashboard/guia/passeios" 
            className="bg-orange-500 hover:bg-orange-400 text-gray-950 font-bold px-4 py-2 rounded-sm text-xs uppercase tracking-widest transition"
          >
            Área do Guia
          </Link>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-12">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 bg-orange-500/10 border border-orange-500/20 text-orange-400 px-3 py-1 rounded-full text-xs font-semibold mb-4 uppercase tracking-wider">
            <Compass className="w-3.5 h-3.5" /> Vitrine de Aventuras
          </div>
        ...
        {/* Abas de Filtro (Novos + Regiões Dinâmicas) */}
