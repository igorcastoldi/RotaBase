...
import { Bike, Clock, Users, Tag, Plus, Trash2, MapPin } from 'lucide-react';
...
export default function MeusPasseios() {
  ...
  async function apagarPasseio(id: string) {
    if (!confirm('Tem certeza que deseja apagar este passeio?')) return;
  ...
  // Detecta se estamos no painel de empresa ou guia pelo URL atual
  const isGuia = typeof window !== 'undefined' && window.location.pathname.includes('/guia/');
  const novoLink = isGuia ? '/dashboard/guia/passeios/novo' : '/dashboard/empresa/passeios/novo';
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {passeios.map((passeio) => (
              <div key={passeio.id} className="bg-gray-900 rounded-sm border border-gray-800 overflow-hidden shadow-sm hover:border-gray-700 transition group flex flex-col">
